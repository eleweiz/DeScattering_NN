...
H=128   # replace the x dimension of your own data
W=128   # replace the y dimension of your own data
D=64    # replace the z dimension of your own data

matfn = './Training_SpineN_in_Scarlet.mat'  # replace the training file name
data= h5py.File(matfn)
trainX = data['I_in']
trainX=np.reshape(trainX,trainX.shape + (1,)) 
data=None

matfn1 = './Training_SpineN_out_Scarlet.mat'  # replace the training file name
...
############ train
history =model.fit_generator(training_generator,steps_per_epoch=300,epochs=30,callbacks=[model_checkpoint],validation_data=validation_generator, validation_steps=3)
