...
z_start=1;                                  % The start depth in PSTPM image [um]
...
NN_t=round((length(files)-1)*134);              % total number of data, leave one file for test.
...
nn_z=round(NN_t/(length(files)-1));           % number of patterns generated from each tif: should be integer
Count_nn_tem=1;
...
for i=1:length(files)-1
    ...
    for mm=64:-1:1
        if i<28  % for data having 1 channel: for our data, the first 28 files have 1 channel and other have three channel.
            j=mm;  % loop for depth
        else      % for data having 3 channels
            j=(mm-1)*3+1; % loop for depth
        end                                   % loop for depth：Red Channel image
...
save('Training_SpineN_in_EYFP','I_in')
save('Training_SpineN_out_EYFP','I_out')

disp('Training data saved sucessfully.');
