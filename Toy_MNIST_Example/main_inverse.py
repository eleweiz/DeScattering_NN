...
H=32   # replace the x dimension of your own data
W=32   # replace the y dimension of your own data
D=32   # replace the z dimension of your own data

matfn = './Training_Toy_in.mat'
data= h5py.File(matfn)
trainX = data['I_in']
trainX=np.reshape(trainX,trainX.shape + (1,)) 
data=None

matfn1 = './Training_Toy_out.mat'
...
portion=0.9  # 90% for training, and 10% for validation
...
############ train
history =model.fit_generator(training_generator,steps_per_epoch=200,epochs=5,callbacks=[model_checkpoint],validation_data=validation_generator, validation_steps=20)
